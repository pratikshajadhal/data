...
from etl.datamodel import  FileVineConfig, LeadDocketConfig
from utils import load_config, load_lead_config

load_dotenv()

# - - - - - - - - - - - - - - - - - - - -Filevine - - - - - - - - - - - - - - - - - - - 
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
def start_contact_etl(conf_path):
    selected_field_config = load_config(file_path=conf_path)
...
def start_project_etl(conf_path):
    selected_field_config = load_config(file_path=conf_path)
    print(selected_field_config.projectTypes[0])
    fv_config = FileVineConfig(org_id=selected_field_config.org_id, user_id=selected_field_config.user_id)
...
def start_form_etl(conf_path, project_type, form_name):
    selected_field_config = load_config(file_path=conf_path)
    print(selected_field_config.projectTypes[0])
    fv_config = FileVineConfig(org_id=selected_field_config.org_id, user_id=selected_field_config.user_id)
...
def start_collection_etl(conf_path, project_type, section_name):
    selected_field_config = load_config(file_path=conf_path)
    print(selected_field_config.projectTypes[0])
    fv_config = FileVineConfig(org_id=selected_field_config.org_id, user_id=selected_field_config.user_id)
